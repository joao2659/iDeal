...
module.exports = async (req, res) => {
  ...
  if (!verifySignature(xSignature, xRequestId, paymentId)) {
  ...
  try {
    ...
    if (status === 'approved') {
      const reqRes = await fetch(`${process.env.SUPABASE_URL}/rest/v1/requests?id=eq.${requestId}&select=buyer_id,traveler_id,product_name,request_status`, { headers: supaHeaders });
      const reqRows = await reqRes.json();
      const r = reqRows[0];
      if (r && r.request_status !== 'em_transporte') {
